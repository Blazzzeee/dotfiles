...
function _filesystem.save_file(path, text, callback, is_retry)
	...
	_filesystem.is_file_readable(path, function(is_readable)
		if not is_readable then
			...
			gfile:create_readwrite_async(Gio.FileCreateFlags.NONE, Glib.PRIORITY_DEFAULT, nil,
				function(_, create_result)
			...
			gfile:open_readwrite_async(Glib.PRIORITY_DEFAULT, nil, function(_, io_stream_result)
				...
				file:write_all_async(text, Glib.PRIORITY_DEFAULT, nil, function(_, write_result)
					...
					file:close_async(Glib.PRIORITY_DEFAULT, nil, function(_, file_close_result)
						...
						tostring(file:close_finish(file_close_result)))
						io_stream:close_async(Glib.PRIORITY_DEFAULT, nil,
							function(_, stream_close_result)
								print("file stream closed " ..
								tostring(io_stream:close_finish(stream_close_result)))
								if callback then
									callback(true)
								end
							end, nil)
			    ...
			    callback = function()
				    _filesystem.query_info(path, function(info)
					    if info ~= nil then
						    ...
						    local diff = math.ceil(Glib.DateTime.new_now_local():difference(time) /
						    1000000)
