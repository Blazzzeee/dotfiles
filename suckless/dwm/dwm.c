...
static int gappx = 10; /* default gap size in pixels */
...
char *getclass(Client *c) {
  XClassHint ch = {NULL, NULL};
  char *res_class = NULL;

  if (XGetClassHint(dpy, c->win, &ch)) {
    if (ch.res_class)
      res_class = strdup(ch.res_class); // or ch.res_name
    if (ch.res_name)
      XFree(ch.res_name);
    if (ch.res_class)
      XFree(ch.res_class);
...
void drawbar(Monitor *m) {
  ...
  if ((w = m->ww - tw - x) > bh) {
    if (m->sel) {
      drw_setscheme(drw, scheme[SchemeNorm]); // Always use SchemeNorm
      char *class = getclass(m->sel);
      drw_text(drw, x, 0, w, bh, lrpad / 2, class ? class : m->sel->name, 0);
      if (class)
        free(class);
      if (m->sel->isfloating)
